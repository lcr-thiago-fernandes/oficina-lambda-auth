...
public sealed class ProtecaoContraForcaBruta
{
    ...
    /// <summary>
    /// Origem nula/vazia (TestServer fora da Lambda, ou proxy que não preenche o IP) não
    /// vira um balde global único: a checagem e o registro por origem são pulados e só a
    /// identidade continua contando.
    /// </summary>
    public async Task ExigirPermitidoAsync(string? origem, string? identidade, CancellationToken ct)
    {
        if (!string.IsNullOrEmpty(origem) &&
            await _limitador.ContarFalhasAsync(ChaveOrigem(origem), ct) >= _politica.MaximoPorOrigem)
